...
React Hooks foram lançados há quase dois anos na versão 17 do React e ainda assim não vejo o uso de outros hooks que são muito úteis, por isso gostaria de explicar e dar alguns exemplos do uso de todos os React Hooks.
...
Isso não é um React Hook mas você também pode usar o `React.memo` para evitar que todo componente filho seja rerenderizado desnecessariamente sempre que houver alguma atualização no componente pai.
...
Dessa forma, o elemento pai pode ser capaz de interagir com o _ref_ do elemento filho.
